...
```bash
python-dev-sam-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
python-dev-sam-app$ sam local start-api
python-dev-sam-app$ curl http://localhost:3000/
```
...
```bash
python-dev-sam-app$ sam logs -n HelloWorldFunction --stack-name python-dev-sam-app --tail
```
...
```bash
python-dev-sam-app$ pip install pytest pytest-mock --user
python-dev-sam-app$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name python-dev-sam-app
```
